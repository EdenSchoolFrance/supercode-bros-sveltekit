...
function parseLevel() {
	...
	document
		.querySelectorAll(
			"#my-level h1,#my-level h2,#my-level h3,#my-level h4,#my-level h5,#my-level h6,#my-level p,#my-level input,#my-level button",
		)
		.forEach((el) => {
			...
			switch (tag) {
				case "h1":
					addTile(gx, gy, "ground");
					break;
				case "h2":
					addTile(gx, gy, "brick");
					break;
				case "h3":
					addTile(gx, gy, "question");
					break;
				case "h4":
					addTile(gx, Math.min(gy, ROWS - 2), "pipe_top");
					addTile(gx, Math.min(gy + 1, ROWS - 1), "pipe_body");
					break;
				case "h5":
					addTile(gx, gy, "cloud");
					break;
				case "h6":
					...
					break;
				case "p":
					coins.push({ gx, gy, collected: false });
					break;
				case "input":
					enemies.push(makeEnemy(gx, gy));
					break;
			}
		});
...
function render() {
	...
	if (goalCell) drawFlag(goalCell.gx, goalCell.gy);

	for (const c of coins) if (!c.collected) drawCoin(c.gx, c.gy);
...
function drawCoin(gx, gy) {
	...
	const cy = gy * TILE + TILE / 2 + Math.sin(frame * 0.1 + gx * 0.7) * 3;
	const sh = Math.floor(frame / 9) % 5;
...
function drawPlayer() {
	...
	if (player.invincible > 0 && Math.floor(player.invincible / 5) % 2 === 1)
		return;
